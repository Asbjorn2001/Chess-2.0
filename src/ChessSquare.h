...
public:
    ...
    constexpr ChessSquare(int row, int col) {
        if (row < 0 || row > 7 || col < 0 || col > 7) {
            throw std::out_of_range("Row/col initialized chess square must be 0-7");
        }
    ...
    constexpr ChessSquare(char file, char rank) {
        int col = file - 'a';
        if (col < 0 || col > 7) {
            throw std::out_of_range("Char initialized chess file must be \'a-h\'");
        }

        int row = rank - '1';
        if (row < 0 || row > 7 ) {
            throw std::out_of_range("Char initialized chess rank must be \'1-8\'");
        }
...
public:
    ...
    constexpr OccupantType type() const {
        switch (m_value) {
            case 'p': case 'P': return OccupantType::Pawn;
            case 'n': case 'N': return OccupantType::Knight;
            case 'b': case 'B': return OccupantType::Bishop;
            case 'r': case 'R': return OccupantType::Rook;
            case 'q': case 'Q': return OccupantType::Queen;
            case 'k': case 'K': return OccupantType::King;
            default: return OccupantType::None;
        }
    }

    constexpr ChessColor piece_color() const {
        if (m_value >= 'A' && m_value <= 'Z') return ChessColor::White;
        if (m_value >= 'a' && m_value <= 'z') return ChessColor::Black;
    ...
    friend std::ostream& operator<<(std::ostream& os, const SquareOccupant& sq) {
